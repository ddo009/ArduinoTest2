//
// Created by Donghaechoi on 2016. 5. 27..
//
  ...
  { // the wifi shield has data available
    ...
    if (wiflyUart.find("*OPEN*")) // see if the data available is from an open connection by looking for the *OPEN* string
    {
      ...
      while (currentLineIsBlank) {
        ...
        Serial.println(analogRead(IlluminationPin));
        Serial.println("------------------");
        ...
        wiflyUart.print(", \"illumination\": ");
        wiflyUart.print(analogRead(IlluminationPin));
        wiflyUart.print(" }");
        break;
      }
    }
